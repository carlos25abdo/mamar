  ...
  constructor() {
    ...
    this.translations = {
      ar: {
        ...
        'about-title':' مرحبًا بكم في شركة مرمرة',
        'about-desc':' وجهتكم الموثوقة لمستلزمات الرخام والجرانيت في ليبيا! منذ تأسيسنا في عام 2018، ونحن نكرس جهودنا لتقديم منتجات استثنائية تتميز بالجودة والأناقة.'
,
        'about-desc1':' نحن لا نقدم فقط مستلزمات الرخام والجرانيت ، بل تشمل الملحقات الاحترافية للفنيين والمواد اللاصقة الفائقة ، مما يضمن لك تنفيذ مشاريعك بأعلى معايير الاحتراف.',
...
initMobileNavigation() {
  ...
  initCategorySliders() {
    ...
    if (!sliders.length) {
      console.warn('No category sliders found');
      return;
    }
    ...
    sliders.forEach(slider => {
      ...
      if (!track || !cards.length) {
        console.warn(`Slider track or cards not found for category: ${category}`);
        return;
      }
      ...
      const getVisibleCards = () => {
        ...
        const cardsPerView = Math.floor(sliderWidth / (cardWidth + gap));
      ...
      console.log(`Category slider initialized for: ${category}`);
    });
    
    console.log(`Category sliders initialized for ${sliders.length} categories`);
  }
